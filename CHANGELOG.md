...
- In an encrypted room, transcripts are sealed before they leave the browser and a transcript that arrives unencrypted is dropped; client monitoring samples are not sent. The README notes that the browser's speech recognition itself sends audio to the browser vendor's service
...
- Refuses the client monitoring data channel in an encrypted room, so an outdated or altered client cannot hand the media node readable samples
