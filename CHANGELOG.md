...
- In an encrypted room, transcripts sent to the other participants are encrypted under the same keys. Client monitoring samples, where the operator has configured them, are sent to the media node as in any other room; the client settings `obfuscateDisplayName` and `obfuscateRoomName` keep names out of them. See E2EE.md for what is and is not covered
...
- New `obfuscateRoomName` setting: the client monitoring samples then name the room by its session id instead of its name, next to `obfuscateDisplayName` for the display name. Both are off by default
