...
- Server-side recording, streaming and transcription, done by an outside service. A tenant configures a provider per kind of job in the tenant editor (the bot access token, the provider's https API address and the API key it issued, stored encrypted with a new `bots.encryptionKey` in the management server config); moderators then get "Start server recording", "Start live stream" and "Start server transcription" in the More menu, confirmed in a dialog naming the provider and, in an end-to-end encrypted room, what it will be able to see. Everyone in the room sees a running job in the top bar, a blinking red dot for a recording and its own icon for a stream or a transcription; a moderator sees a row per job in the bot menu and stops it there, which asks the provider to finish properly instead of cutting its browser off. A job survives a room-server restart: the provider is not told to stop, and its browser rejoins and picks the job up again. Only a signed-in moderator can start a job: the start call carries the people to notify, the room owners and the starting moderator, and the tenant's language, a new setting in the tenant editor that is also the starting language of a new meeting invitation; the provider sends the notice when the recording is ready. Every job also carries the main room's session id, so a provider can put the recordings of a meeting and its breakout rooms into one notice. A transcriber receives audio only. Providers are read once per room, so changes apply to the next room. Where client monitoring samples are collected, a bot now sends its own, marked as a bot's, so what a recorder received can be seen alongside what the participants sent. Without a configured provider nothing changes anywhere, and a bot started by hand works as before. The management server must be upgraded first. New migration; runs on Postgres and MySQL. The contract a provider implements is documented in BOT-PROVIDER-API.md
